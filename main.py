...
CORPUS_PATH = ["./data/wikitext-2/wiki.test.tokens"]
...
for epoch in range(NUM_EPOCHS):
    ...
    for i in range(1, update_time + 1):
        ...
        word_u_variable, word_v_variable, words_co_occurences, words_weights = GloVe.next_batch(BATCH_SIZE) # when it is not in parallel
    ...
    np.savez(
        ...
        # )
        word_embeddings_array=GloVe.embedding(), 
        word_to_index=GloVe.word_to_index,
        index_to_word=GloVe.index_to_word
        )
logger.info("Done")
